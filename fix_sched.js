...
const path = 'C:/Users/User/Documents/Roots Astro Tele/frontend/src/pages/AstrologerDashboard.jsx';
let content = fs.readFileSync(path, 'utf8');

const old_style_start = '<style>{`\n                                    /* ── Base (mobile) card layout ── */';
const old_style_end = '                                    @media (min-width: 992px) {';
...
if (content.includes(old_style_start) && content.includes(old_style_end)) {
    const s = content.indexOf(old_style_start);
    const e = content.indexOf(old_style_end);
    content = content.substring(0, s) + new_style + content.substring(e);
}

const old_jsx_target = `                                                        {/* Time range row */}
